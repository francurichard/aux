...
import signal
from injecter import start_injecting
...
def get_mac(ip_address):
    arp_frame = Ether(dst="ff:ff:ff:ff:ff:ff") / ARP(op=1, pdst=ip_address)
    resp, _ = srp(arp_frame)
    for s, r in resp:
        return r[Ether].src
...
#Hijacking poison
print('here')
hij_thread = threading.Thread(target=start_injecting)
hij_thread.start()
